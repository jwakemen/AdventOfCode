...
def run_program():
    logger.info(f"Starting program  - machine: {machine}")
    logger.info(f"Instructions: {machine['M']} - Length: {len(machine['M'])}")
    while machine["I"] < len(machine["M"]):
        ...
        logger.info(f"After running operation - machine: {machine}")
...
def main():
    ...
    # write the output
    final_output = f"{aoc_name} - output: {output}\n"
